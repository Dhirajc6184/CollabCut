        ...
        {clips.map((clip, idx) => {
          ...
          return (
            <div key={clip.id} title={(clip.op + ": " + formatTime(clip.start) + " to " + formatTime(clip.end))}
              style={{ ...st.clip, left: (pct(clip.start) + "%"), width: (Math.max(w, 0.4) + "%"), background: color + "28", borderColor: color, outline: clipDrag && clipDrag.idx === idx ? ("1px solid " + color) : "none" }}
...
export default function VideoEditor({ project, user, token, onBack }) {
  ...
  const onScrubMouseDown = useCallback(e => {
    ...
    const onUp   = ev => { scrubDragRef.current = false; setScrubDragging(false); scrubTimeFromEvent(ev); window.removeEventListener("mousemove", onMove); window.removeEventListener("mouseup", onUp); };
    window.addEventListener("mousemove", onMove);
    window.addEventListener("mouseup",   onUp);
  }, [scrubTimeFromEvent]);
        ...
        <main style={st.main}>
          ...
          {uploadedFile && (
            <div style={st.controls}>
              ...
              <button style={st.newBtn} onClick={() => { setUploadedFile(null); setPipeline([]); setClips([]); setResult(null); setError(null); setCurrentTime(0); setDuration(0); setIsPlaying(false); }}>new video</button>
